...
# webroot 폴더 존재 여부 확인 및 생성
if ! test -d /www/$webroot_folder ; then
    echo "webroot folder '/www/$webroot_folder' does not exist. Creating..."
    mkdir -p /www/$webroot_folder
    echo "Created webroot folder: /www/$webroot_folder"
    chown -R www-data:www-data /www/$webroot_folder
    chmod -R 755 /www/$webroot_folder
    echo "Set permissions for webroot folder: /www/$webroot_folder (owner: www-data, mode: 755)"
else
    echo "webroot folder '/www/$webroot_folder' already exists."
fi
